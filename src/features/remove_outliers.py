...
df[outlier_columns[:3] + ["label"]].boxplot(by="label", figsize=(20,10), layout=(1,3)) ## acc columns
df[outlier_columns[3:] + ["label"]].boxplot(by="label", figsize=(20,10), layout=(1,3)) ## gyr columns
...
def plot_binary_outliers(dataset, col, outlier_col, reset_index):
    ...
    # Plot non outliers in default color(blue)
        ax.plot(
...
### this function marks true if outlier false if not and makes the outlier column
...
# --------------------------------------------------------------
# Chauvenets criteron (distribution based) ----removes the outlier if the probability of its appearance is less than 1/cn
# --------------------------------------------------------------
...
df[outlier_columns[:3] + ["label"]].plot.hist(by="label", figsize=(20,20), layout=(3,3)) ## acc columns
df[outlier_columns[3:] + ["label"]].plot.hist(by="label", figsize=(20,20), layout=(3,3)) ## gyr columns
...
for col in outlier_columns:
    ...
    plot_binary_outliers(dataset,col,col+"_outlier",reset_index=True)
### notice that there are less outliers here 
...
for col in outlier_columns:   ### gives fewer outliers as compared to iqr so better
